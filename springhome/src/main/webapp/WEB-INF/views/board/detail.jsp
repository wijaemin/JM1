...
<script>
$(function(){
	// 목표: 댓글 등록을 누르면 입력정보로 ajax 통신을 통해 댓글 등록 처리
	//(주의) form은 전송이 되면 안된다
	...
	//- 삭제 버튼을 누르면 확인창 출력 후 삭제하도록 처리
 	function loadList(){
		...
		$.ajax({
		 ...
		 success:function(response){
				//화면 청소
				//$(".reply-list").remove();//자기 자신까지 삭제(하면안된다)
			...
			for(var i=0; i <response.length;i++){
				...
				if(memberId.length ==0 || memberId != reply.replyWriter){//내가 작성한 댓글이 아니라면
					//버튼 삭제
				...
				$(htmlTemplate).find(".btn-edit")
					...
					//완료(등록)버튼 처리
					//-editHtmlTemplate 자체가 form이므로 추가 탐색을 하지 않음(find안됨)
