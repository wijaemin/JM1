...
public class BoardDaoImpl implements BoardDao {
	...
	public void insert(BoardDto boardDto) {
		String sql = "insert into board(board_no, board_writer, board_title, " + "board_content, board_group, "
				+ "board_parent, board_depth) values(?, ?, ?, ?, ?, ?, ?)";
		Object[] data = { boardDto.getBoardNo(), boardDto.getBoardWriter(), boardDto.getBoardTitle(),
				boardDto.getBoardContent(), boardDto.getBoardGroup(), boardDto.getBoardParent(),
...
//	}
	...
	public List<BoardListDto> selectListByPage(String type, String keyword, int page) {
		...
		String sql = "select * from (" 
				+ "	select rownum rn, TMP.* from (" 
				+ "select * from board_list where instr("
				+ type + ",?)>0 " 
				+ "connect by prior board_no= board_parent " 
				+ "start with board_parent is null "
				+ "order siblings by board_group desc, board_no asc" 
	...
	public int countList(String type, String keyword) {
		String sql = "select count(*) from board " + "where instr(" + type + ",?)>0"; // 구문의 일부분은 홀더(물음표)처리를 안함
		Object[] data = { keyword };
	...
	public int countList(PaginationVO vo) {
		if (vo.isSearch()) {
			String sql = "select count(*) from board " + "where instr(" + vo.getType() + ",?)>0"; // 구문의 일부분은 홀더(물음표)처리를
																									// 안함
