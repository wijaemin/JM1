...
<head>
    ...
    <style>
    	...
    	::-webkit-scrollbar-thumb{
    		background: var(--bs-info);/* 스크롤바 색상 */
    	}
         ...
         if(data.clients){//목록처리
            ...
            for(var i = 0; i < data.clients.length; i++){
               $("<li>").addClass("list-group-item d-flex justify-content-between align-items-center")
                        ...
                        .append($("<span>").addClass("badge bg-primary badge-pill").text(data.clients[i].memberLevel))
                        .appendTo(ul)
//                $("<div>").text(data.clients[i].memberId).appendTo(".clients-list");
         ...
         if(data.dm ==true){//DM이라면
        	 ...
        	 else{//target이 없다면(내가 DM을 받아서 찍히는 메세지)
      ...
      //.btn-userlistfmf 누르면 사용자 목록에 active를 붙였다 떼었다 하도록 처리
