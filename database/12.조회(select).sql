...
--<문자열 조건>
--문자열도 같음을 =로 처리한다
--유사검색은 like와 insetr()로 처리할 수 있다
-- 시작검색만큼은 반드시 like를 써야할 정도로 압도적인 차이가 발생
-- 나머지 성능은 전체적으로 instr가 우수하다
...
select * from product where instr(name,'이')>1;
...
select * from product
where to_char(made,'yyyy-mm') in ('2019-07','2019-08','2019-09','2019-10','2019-11','2019-12');
...
select * from product where made between 
sysdate -365 and sysdate ;

--(응용)시간까지 고려(시작일 00시부터 종료일 23시 59분까지)
select * from product
	where made between
	to_date(
	to_char(sysdate-2000,'yyyy-mm-dd') ||' '|| '00:00:00',
