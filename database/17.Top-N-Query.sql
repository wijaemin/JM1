...
--오라클은 rownum,이라는 값을 조회 시 사용할 수 있다
...
select * from(
	select rownum rn ,TMP.* from(
		select *from book order by book_price desc)TMP
)where rn between 1 and 5;
--(Q) 제목이 가장 짧은 책 Top 3 조회
select * from book;
select * from(
	select rownum rn ,TMP.* from(
		select *from book order by length(book_title) asc )TMP
)where rn between 1 and 3;
--(Q) 출간한지 가장 오래된 책 6~10등 조회
select * from book;
select * from(
	select rownum rn ,TMP.* from(
		select *from book order by book_publication_date asc )TMP
